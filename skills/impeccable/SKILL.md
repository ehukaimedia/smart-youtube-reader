...
Two files, case-insensitive. The loader looks at the project root by default and falls back to `context/` and `docs/` if the root is clean. Override with `IMPECCABLE_CONTEXT_DIR=path/to/dir` (absolute or relative to cwd).
