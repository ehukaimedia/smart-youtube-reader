...
2. Simplify archive chat routing to Ollama.
3. Expose only the Gemma4-based `smart-reader:latest` archive model.
4. Update `smart-reader:latest` prompt instructions for compact no-fluff learning chapters.
...
10. Update architecture playground and README so the documented flow matches the single-model Ollama implementation.
