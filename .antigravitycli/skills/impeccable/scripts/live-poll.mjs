 ...
 *
 * Usage:
 *   antigravitycli impeccable poll                         # Block until browser event, print JSON
 *   antigravitycli impeccable poll --timeout=600000        # Custom timeout (ms); default is long-poll friendly
 *   antigravitycli impeccable poll --reply <id> done       # Reply "done" to event <id>
 *   antigravitycli impeccable poll --reply <id> error "msg" # Reply with error
 */
...
function readServerInfo() {
  try {
    ...
    console.error('No running live server found. Start one with: antigravitycli impeccable live');
    process.exit(1);
  }
}
...
Options:
  ...
  // Reply mode: antigravitycli impeccable poll --reply <id> <status> [--file path] [message]
  ...
  if (replyIdx !== -1) {
    ...
    if (!id) {
      console.error('Usage: antigravitycli impeccable poll --reply <id> <status> [--file path] [message]');
      process.exit(1);
    }
    ...
    try {
      ...
      if (err.cause?.code === 'ECONNREFUSED') {
        console.error('Live server not running. Start one with: antigravitycli impeccable live');
      } else {
  ...
  try {
    while (true) {
      ...
      if (res.status === 401) {
        ...
        console.error('Try restarting: antigravitycli impeccable live stop && antigravitycli impeccable live');
        process.exit(1);
      }
